...
class Sprite:
    ...
    def collide(self, other_object):
        if dist(other_object.pos, self.pos) <= other_object.radius + self.radius:
            return True
        else:
            return False
...
def group_group_collide(rock_set, missile_set):
    no_of_hits = 0
    remove = set([])
    for a_rock in rock_set:
        if group_collide(missile_set, a_rock):
            no_of_hits += 1
            remove.add(a_rock)
    rock_set.difference_update(remove)
    return no_of_hits
