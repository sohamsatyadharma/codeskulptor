...
class Ship:
    ...
    def update(self):
        ...
        if self.thrust:
            self.vel[0] += self.forward[0] * 1.2
            self.vel[1] += self.forward[1] * 1.2
...
class Sprite:
    ...
    def shoot(self, ship):
        self.pos[0] = ship.pos[0] + 44 * angle_to_vector(ship.angle)[0]
        self.pos[1] = ship.pos[1] + 44 * angle_to_vector(ship.angle)[1]
        self.vel[0] = ship.vel[0] + ship.forward[0] * 5
        self.vel[1] = ship.vel[1] + ship.forward[1] * 5
        missile_sound.play()
...
def rock_spawner():
    global a_rock
    pos = [random.randrange(40, 760), random.randrange(40, 560)]
